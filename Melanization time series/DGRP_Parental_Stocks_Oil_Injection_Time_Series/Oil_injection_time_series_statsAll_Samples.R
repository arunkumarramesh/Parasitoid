...
## Save plot
pdf(file="Oil_Melanization_Rec_Lines.pdf",height=3.4,width=4.1)
p1
dev.off()
##stats
#Stats - to correct
...
##simpler model is sufficient, use m1
m2 <- glm(data=data, cbind(data$Melanised, data$Non_Melanised)~DGRP_Line + Duration + DGRP_Line*Duration + Dissection_Date + Injection_Date + Dissection_Time + Injection_Time,family=binomial())
summary(m2)
anova(m2,test="Chisq")
