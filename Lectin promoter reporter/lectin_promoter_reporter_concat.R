...
p = ggplot(data, aes(x=Label, y=log10(Value/Protein_Quantity_ug)))+
  ...
  geom_point(aes(color = Treatment), position = position_jitter(width = 0.2, height = 0), 
             size = 0.5)+
  ylab(expression(log[10]*"(Relative Fluorescence Intensity / Total Protein (ug))")) + 
  theme(text = element_text(size = 10)) + 
  scale_color_hue(labels = c("Uninfected", "Infected")) +
  xlab("Genotype")
...
for(i in 1:9) {
  p <- p + annotate("text", label = labels_noinf[[i]],
                    x = i, y = 5.25, size = 4, color = "#F8766D", fontface = "bold")
}

for(i in 1:9) {
  p <- p + annotate("text", label = labels_inf[[i]],
                    x = i, y = 5.5, size = 4, color = "#00BFC4", fontface = "bold")
}
...
ggsave("lectin-24A promoter reporter assay Tukey.pdf", width = 5.3, height = 4)
